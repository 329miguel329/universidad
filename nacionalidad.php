<?php
	...
	$nac = $conn -> Select("SELECT id id_nac, name nombre_nacionalidad,(SELECT COUNT(*) FROM estudiante est WHERE est.id_nacionalidad = id_nac)num_est FROM nacionalidad nac WHERE 1");
...
<body>
	<div class="d-principal" align="center">
		...
		<?php
			if($nac["num_results"] > 0){
				for($i = 0; $i < $nac["num_results"]; $i++){
					$consulta = $conn -> Select("SELECT est.id id_est, est.nombre nombre, est.apellido apellido, est.edad edad, gen.id id_gen, gen.name genero, sem.id id_sem, sem.name semestre, car.id id_car, car.name carrera, nac.id id_nac, nac.name nacionalidad
												...
												WHERE nac.id=" . $nac["results"][$i]["id_nac"]);
			...
			<div class="panel panel-default">
				<div class="panel-heading" role="tab" id="dh-<?php echo $nac["results"][$i]["id_nac"]; ?>">
					<h4 class="panel-title">
						<a class="collapsed" role="button" data-toggle="collapse" data-parent="#accordion" href="#<?php echo $nac["results"][$i]["id_nac"]; ?>" aria-expanded="false" aria-controls="<?php echo $nac["results"][$i]["id_nac"]; ?>">
							<?php echo "<h4>Nacionalidad: " . $nac["results"][$i]["nombre_nacionalidad"] . "<br />Cantidad de estudiantes: " . $nac["results"][$i]["num_est"] . "<br /></h4>";?>
						</a>
					</h4>
				</div>
				<div id="<?php echo $nac["results"][$i]["id_nac"]; ?>" class="panel-collapse collapse" role="tabpanel" aria-labelledby="dh-<?php echo $nac["results"][$i]["id_nac"]; ?>">
